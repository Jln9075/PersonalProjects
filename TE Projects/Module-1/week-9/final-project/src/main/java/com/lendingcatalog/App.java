...
public class App {
    ...
    private void initialize() {
        // Requirement: Data transformation
            ...
            for(String holdingString : data){
                String[] stringArr = holdingString.split(FIELD_DELIMITER);
                String name = stringArr[0] + " " + stringArr[1];
                catalog.put(name,catalogValues(stringArr[2]));
            }


    }

    private List<String> createCatalogData(){

        try {
            return FileStorageService.readContentsOfFile(FILE_BASE_PATH + "members.dat");
        }
    ...
    private List<CatalogItem> catalogValues(String inputValues){
        List<CatalogItem> membersItems = new ArrayList<>();

        try{
            File dataFile = new File(FILE_BASE_PATH + inputValues);
            Scanner inputData = new Scanner(dataFile);
            while(inputData.hasNextLine()){
                String[] currentLine = inputData.nextLine().split(FIELD_DELIMITER);
                String itemType = currentLine[0];
                String itemName = currentLine[1];
                String itemMaker = currentLine[2];



                if(itemType.equals("book")){
                    LocalDate publishedDate = LocalDate.parse(currentLine[3]);
                    Book newBook = new Book(itemName, itemMaker, publishedDate);
                    newBook.registerItem();
                    membersItems.add(newBook);

                } else if(itemType.equals("tool")){
                    int toolCount = Integer.parseInt(currentLine[3]);
                    Tool newTool = new Tool(itemName, itemMaker, toolCount);
                    newTool.registerItem();
                    membersItems.add(newTool);

                } else if(itemType.equals("movie")){
                    LocalDate releaseDate = LocalDate.parse(currentLine[3]);
                    Movie newMovie = new Movie(itemName, itemMaker, releaseDate);
                    newMovie.registerItem();
                    membersItems.add(newMovie);

                } else{
                    throw new Exception("This item type does not exist");
                }
            }
        }
        catch(Exception e){
