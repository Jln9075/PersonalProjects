...
public class PracticePart1 {
	...
	*/
	public List<Integer> Q10_reverseList(List<Integer> numbers) {
		List testList = new ArrayList<>(numbers.size());
		if(numbers.size() == 0){
			return testList;
		}
		for(int i = numbers.size(); i > 0; i--){
			int j = 1;
			testList.add(numbers.get(i));
			j++;
		}
	...
	*/
	public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
		...
		for(String holdingString : suggestedPets){
			if(petMap.containsKey(holdingString)){
				int addPetNum = 1 + petMap.get(holdingString);
				petMap.put(holdingString, addPetNum);
			} else{
				petMap.put(holdingString, 1);
			}
		}
