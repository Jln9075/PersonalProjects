...
public class AssessmentPart1 {
	...
	*/
	public double Q01_calculateFraction(int numerator, int denominator) {
		double finalNum = (double) numerator / denominator;
		return finalNum;
	}
	...
	*/
	public String Q06_firstNCharacters(String originalString, int numCharacters) {
		...
		originalString = originalString.substring(0, numCharacters);
		return originalString;
	}
	...
	*/
	public int[] Q10_swapFirstAndLastElements(int[] arrayOfInts) {
		int firstNum = arrayOfInts[0];
		int lastNum = arrayOfInts[arrayOfInts.length-1];

		arrayOfInts[0] = lastNum;
		arrayOfInts[arrayOfInts.length-1] = firstNum;
	...
	*/
	public Map<String, Integer> Q11_mealCount(String[] mealOrders) {
		...
		for(String holdingString : mealOrders){
			if(foodMap.containsKey(holdingString)){
				int currNum = foodMap.get(holdingString) + 1;

				foodMap.replace(holdingString, currNum);

			}
			else {
				foodMap.put(holdingString, 1);
			}
		}
		return foodMap;
	}
